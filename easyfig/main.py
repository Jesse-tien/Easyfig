# Author: Yu-Xin Tian, 2026-08-20 Version 3.7.0
...
def data_lines(data,
    ...
    # ========== 关键修改 1：横轴以最长曲线为准 ==========
    max_len = max(len(data[name]) for name in data_names)
    the_x = list(range(1, max_len + 1))

    # 原代码这里是 the_x[1]，只到 2，应改为最后一个
    ...
    if label_x is None:
        label_x = [str(x_) if i_ % labelevery == 0 else ''
                   for i_, x_ in enumerate(the_x)]
    else:
        # 如果外部传入的 label_x 长度不足，先补齐，避免索引越界
        label_x = list(label_x)
        if len(label_x) < max_len:
            label_x += [str(x_) for x_ in the_x[len(label_x):]]
        label_x = [str(label_x[i_]) if i_ % labelevery == 0 else ''
    ...
    if data_num == 1:
        ...
        ax.plot(x_data, ys, linestyle='-', c='k', linewidth=linewidth,
        ...
        for j, em in enumerate(data_names):
            ...
            # ========== 关键修改 3：markevery 按本条曲线长度计算 ==========
            # 避免短曲线因为最长曲线过长而导致标记过稀
            if len(x_data) > 10:
                markevery = int(len(x_data) / 10)
                ax.plot(x_data, ys, linestyle=the_ls, c=colors[j], linewidth=linewidth,
                        marker=markers[j], markersize=markersize, markevery=markevery,
                        label=em)
            else:
                ax.plot(x_data, ys, linestyle=the_ls, c=colors[j], linewidth=linewidth,
                        marker=markers[j], markersize=markersize, label=em)
...
def makefig():
    ...
    def create_dszz(parent):
        ...
        # 底部文本
        tk.Label(ds_panel,text="如果有任何建议，欢迎邮箱反馈：tianyuxin@mail.neu.edu.cn\n作者：东北大学 田雨鑫\n2026-08-20",font=("宋体", 13)).pack(side="top", pady=5)
    ...
    # 初始化主窗口
    root = tk.Tk()
    root.title("EasyFig仿真绘图向导 V3.7.0 - 作者：东北大学 田雨鑫博士 - 让天下没有难用的工具！")
